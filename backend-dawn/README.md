...
播种固定 fixture → 起后端 → 256 条响应逐字节比对。四套脚本里 `contract_qiniu.py` 另起一个
...
- 测试：`dawn test .`（`src/` 共 42 个 Dawn 源文件、93 个本仓单测，连 web/json/sha2 三个包
  共 161 个；`use java` import 共 61 条、分布在 12 个文件；无需 .env / 库 /
...
- `api/webdav.dawn`：`Destination` 只把 path-absolute 或 authority 与请求 `Host` 归一后相同的
  ...
  fragment 都返回 400，不会只取 path 后写入本地。
