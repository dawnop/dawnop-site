...
EXPECTED_ROWS = [
    "repo-file-ancestor-fail-open|test|insert_folder rejects an existing file ancestor",
    "repo-immediate-parent-only|test|insert_file checks ancestors above an existing parent row",
    ...
    "fm-skip-reparent-validation|contract|fm.reparent.preflight",
...
def self_test():
    ...
    expect_rejected(
        "unknown-role",
        [*base[:-1], "webdav-missing-parent-fail-open|unknown|new owner"],
        EXPECTED_MUTANTS,
        "unknown matrix role",
    )
    expect_rejected(
