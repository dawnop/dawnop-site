...
def test_upload_into_subfolder_creates_parents(client, auth_headers, stub_qiniu):
    r = _upload(
        client, auth_headers, "qiniu://docs/img", "cover.png", b"\x89PNG", "image/png"
    )
...
def test_copy_then_delete_subtree(client, auth_headers, stub_qiniu):
    ...
    r = client.post(
        "/api/fm/delete",
        json={"path": "qiniu://", "items": [{"path": "qiniu://src", "type": "dir"}]},
        headers=auth_headers,
    ).json()
...
def test_preview_via_header_and_token_query(client, auth_headers, stub_qiniu):
    ...
    r = client.get(
        "/api/fm/preview?path=qiniu://hi.txt",
        headers=auth_headers,
    ...
    r2 = client.get(
        f"/api/fm/preview?path=qiniu://hi.txt&token={token}", follow_redirects=False
    )
    assert r2.status_code == 302 and "token=sig" in r2.headers["location"]
    # 下载：签名 URL 带 attname（强制附件下载）
    d = client.get(
        f"/api/fm/download?path=qiniu://hi.txt&token={token}", follow_redirects=False
    )
    assert d.status_code == 302 and "attname=hi.txt" in d.headers["location"]
    # 未认证仍拒绝
    assert (
        client.get(
            "/api/fm/preview?path=qiniu://hi.txt", follow_redirects=False
        ).status_code
        == 401
    )
...
def test_search_and_save(client, auth_headers, stub_qiniu):
    ...
    deep = client.get(
        "/api/fm/search?path=qiniu://&filter=beta&deep=1", headers=auth_headers
    ).json()
...
def test_stats_prefers_remote_when_larger(
