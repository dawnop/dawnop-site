...
<template>
  <div class="demo">
    ...
    <svg :viewBox="`0 0 ${W} 226`" class="stage">
      ...
      <g v-for="i in T" :key="'t' + i">
        <rect
          ...
          height="22"
          rx="4"
          :fill="paletteSoft[binOf[i - 1]]"
          :stroke="binColor(binOf[i - 1])"
        />
      </g>
    ...
    <p class="cap">
      ...
      同一 bin 上高度并发。<b>层级聚合</b>先在 block 的 shared memory 里聚出局部直方图（便宜的
      shared 原子）， 最后只用<b>常数 {{ B }} 次</b>全局原子合并——全局争抢从「每元素一次」降到「每
      block 几次」。 跨 G 个 block 时，全局原子从 16G 降到 4G。
    </p>
  </div>
</template>
