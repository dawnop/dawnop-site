...
<template>
  <div class="demo">
    ...
    <div class="params">
      示例参数：<b>D=8</b> → bins=<b>256</b> · 每趟候选 <b>÷256</b> · passes ≤ ⌈32/8⌉ =
      <b>{{ PASSES }}</b> · prefix 每趟锁定 8 bit
    </div>
    ...
    <svg :viewBox="`0 0 ${W} 248`" class="stage">
      ...
      <path
        ...
        marker-end="url(#rx-arrow)"
      />
      <text :x="(cyc[0].x + cyc[3].x) / 2" :y="boxB + 50" class="op" :class="cycSt">
        下一趟：候选 ÷256，prefix 多锁 8 bit
      </text>
      ...
      <g v-for="b in cyc" :key="b.id">
        <rect
          ...
          :class="cycSt"
        />
        <text :x="b.x" :y="b.tag ? yBox - 2 : yBox + 4" class="ntxt" :class="cycSt">
          {{ b.label }}
        </text>
        ...
        <rect
          ...
          :x="b.x - cW / 2 + 10"
          :y="boxB - 14"
          :width="(cW - 20) * candFrac"
          height="6"
          rx="3"
          class="candbar"
          :class="cycSt"
        />
        <text v-if="b.id === 'cand'" :x="b.x" :y="boxB - 18" class="cnt" :class="cycSt">
          {{ candLabel }}
        </text>
      </g>
    ...
    <p class="cap">
      radix select 是一条<b>多趟缩并</b>流水：每趟对当前候选集按 8 bit 统计 <b>histogram</b>，从最高
      bin 向下累加定位第 k 大所在的 <b>pivot bin</b>， 再用 <b>filter</b> 把落在 pivot bin
      的候选紧凑搬到下一趟——候选规模随之 <b>÷256</b>，prefix 每趟锁定 8 bit。至多 4 趟比特用尽，
      <b>collect</b> 扫原始数组取出所有不小于阈值的元素即 top-k。片上只一个 256-bin histogram、<b
        >与 k 无关</b
      >，故能扛大 k 与任意分位数。
    </p>
  </div>
...
.arrow.active {
  ...
  animation: rx-flow 0.7s linear infinite;
}
.arrow.done {
  stroke: #aedcc6;
}
...
.loop.active {
  ...
  animation: rx-flow 0.6s linear infinite;
}
.loop.done {
  stroke: #aedcc6;
}
