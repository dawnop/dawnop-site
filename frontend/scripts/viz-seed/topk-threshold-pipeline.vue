...
const step = ref(0)
const next = () => {
  if (step.value < PASSES) step.value++
}
const reset = () => {
  step.value = 0
}
...
const bisectSt = computed(() =>
  step.value === 0 ? 'idle' : step.value < PASSES ? 'active' : 'done',
)
const collectSt = computed(() => (step.value < PASSES ? 'idle' : 'active'))
const stOf = (id) =>
  id === 'load' ? loadSt.value : id === 'bisect' ? bisectSt.value : collectSt.value
...
const caption = computed(() => {
  if (step.value === 0)
    return '起点：B 行各由一个 warp 处理，整行载入 register，初始区间 = [行 min, 行 max]'
  if (step.value < PASSES)
    return `二分第 ${step.value} 轮：count_ge(τ) 与 k 比较 → 收窄 [lo,hi]（所有 warp 同步推进）`
...
<template>
  <div class="demo">
    ...
    <div class="params">
      示例参数：<b>B 行</b> · <b>R</b>/行 · L = R/32 · 一 warp 一行 · 二分
      <b>×ITERS≈8 轮</b>（lockstep，无尾部发散）· 与 <b>k 无关</b>
    </div>
    ...
    <svg :viewBox="`0 0 ${W} 196`" class="stage">
      ...
      <g v-for="n in nodes" :key="n.id">
        <rect
          ...
          :class="stOf(n.id)"
        />
        <text :x="n.x" :y="n.tag ? yBox - 2 : yBox + 4" class="ntxt" :class="stOf(n.id)">
          {{ n.label }}
        </text>
        <text v-if="n.tag" :x="n.x" :y="yBox + 13" class="tag" :class="stOf(n.id)">
          {{ n.tag }}
        </text>
      </g>
    ...
    <p class="cap">
      二分阈值法是一条<b>大批量、强 lockstep</b> 的流水：<b>B 行</b>各由一个 warp 处理，整行载入
      register（<b>vals[L]</b>）， 求出初始区间后<b>二分一个阈值 τ</b>——每轮 <b>count_ge(τ)</b> 用
      ballot + popc 数出过线者、与 k 比较来收窄 [lo,hi]， 所有 warp
      轮数一致、<b>无尾部发散</b>。收敛后用 ballot 前缀和紧凑收集 top-k。全程无 shared、无 atomic，
...
.arrow.active {
  ...
  animation: th-flow 0.7s linear infinite;
}
.arrow.done {
  stroke: #aedcc6;
}
...
.loop.active {
  ...
  animation: th-flow 0.6s linear infinite;
}
.loop.done {
  stroke: #aedcc6;
}
