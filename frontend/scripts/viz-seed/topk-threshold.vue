...
function next() {
  ...
  s.iter += 1
  if (!earlyStop.value && c === k.value) {
    s.finished = true
    return
  }
  if (c > k.value)
    s.lo = s.tau // 选中太多 → 抬高阈值
  else s.hi = s.tau // 选中太少 → 降低阈值
  s.tau = (s.lo + s.hi) / 2
  if (earlyStop.value && s.iter >= MAXITER) {
    s.finished = true
  }
  ...
  <div class="demo">
    <div class="bar">
      <div class="btns">
        ...
        <label class="chk">
          <input type="checkbox" v-model="earlyStop" @change="onParam" /> early-stop（{{ MAXITER }}
          轮近似）
        </label>
      </div>
    </div>
    ...
    <div class="metrics">
      迭代 <b>{{ st.iter }}</b
      ><span class="sep">·</span> 阈值 τ <b>{{ st.tau.toFixed(1) }}</b
      ><span class="sep">·</span> 选中 <b :class="{ bad: curCount !== k }">{{ curCount }}</b> / 目标
      {{ k }}
      <span class="sep">·</span>
      <span v-if="st.finished && err === 0" class="ok">命中精确 top-{{ k }}</span>
      <span v-else-if="st.finished" class="warn">近似停手，误差 {{ err }} 个</span>
      <span v-else class="hint">{{
        curCount > k ? '选中过多 → 抬高 τ' : curCount < k ? '选中过少 → 降低 τ' : '计数已等于 k'
      }}</span>
    </div>
    ...
    <svg :viewBox="`0 0 ${W} 150`" class="stage">
      ...
      <g
        ...
        :key="i"
        class="dot"
        :style="{ transform: `translate(${xOf(v)}px, ${yOf(i)}px)` }"
      >
    ...
    <p class="cap">
      ...
      <b>ballot + popcount</b> 一条指令数出有多少元素过线、无分支。开
      <b>early-stop</b> 固定轮数提前停手得到近似解 （误差通常
      ≤5%）——稀疏化训练里用这点精度换数倍速度非常划算。
    </p>
  </div>
</template>
