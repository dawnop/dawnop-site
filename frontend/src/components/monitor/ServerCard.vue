...
const loadInfo = computed(() => {
  ...
  const level =
    r < 0.7
      ? { label: '空闲', type: 'success' }
      : r < 1
        ? { label: '较忙', type: 'warning' }
  ...
  <el-card shadow="never" class="mon-card">
    <template #header>
      <div class="ch">
        ...
        <el-tag
          v-if="lh?.instance"
          :type="lh.instance.state === 'RUNNING' ? 'success' : 'info'"
          size="small"
          effect="light"
        >
    ...
    <el-alert
      v-if="lh && !lh.configured"
      type="info"
      :closable="false"
      show-icon
      title="未配置腾讯云只读密钥，仅显示本机指标（流量包/到期信息不可用）"
      class="mb"
    />
    ...
    <div v-if="server" class="tiles">
      <div class="tile">
        <div class="tile-l">
          CPU<span v-if="server.cpu_count" class="muted"> · {{ server.cpu_count }} 核</span>
        </div>
        <el-progress
          type="dashboard"
          :width="gaugeSize"
          :percentage="Math.round(server.cpu_percent)"
          :color="barColor(server.cpu_percent)"
        />
      </div>
      <div class="tile">
        ...
        <el-progress
          type="dashboard"
          :width="gaugeSize"
          :percentage="Math.round(server.mem.percent)"
          :color="barColor(server.mem.percent)"
        />
        <div class="tile-sub">
          {{ fmtBytes(server.mem.used) }} / {{ fmtBytes(server.mem.total) }}
        </div>
      </div>
      <div class="tile">
        ...
        <el-progress
          ...
          :color="barColor(server.disk.percent)"
        />
        <div class="tile-sub">
          {{ fmtBytes(server.disk.used) }} / {{ fmtBytes(server.disk.total) }}
        </div>
      </div>
      <div v-if="lh?.traffic" class="tile">
        ...
        />
        <div class="tile-sub">
          {{ fmtBytes(lh.traffic.used) }} / {{ fmtBytes(lh.traffic.total) }}
        </div>
      </div>
    </div>

    <div v-if="server || lh?.instance" class="kv-grid single">
      <div v-if="lh?.instance">
        <span class="k">规格</span>{{ lh.instance.cpu }} 核 {{ lh.instance.memory_gb }} GB
      </div>
      ...
      <div v-if="lh?.traffic">
        ...
        <span v-if="lh.traffic.overflow > 0" class="err">
          · 超额 {{ fmtBytes(lh.traffic.overflow) }}</span
        >
      </div>
      <div v-if="server"><span class="k">运行</span>{{ fmtDuration(server.uptime) }}</div>
      <div v-if="loadInfo" class="load-cell">
        <span class="k">负载</span>
        <el-tag :type="loadInfo.level.type" size="small" effect="light">{{
          loadInfo.level.label
        }}</el-tag>
