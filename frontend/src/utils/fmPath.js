...
// "   "（全是空格）在这一层是合法输入。旧实现在 create 与 rename 两处都 value.trim()，
...
//
// 后端从 2026-08-14 起对写入拒绝「任一路径段 ≠ 其 trim 结果」（400），所以 " a " 与
// "   " 都会被它挡下。这一层**仍然不复制那条规则**：复制等于同一条规则有两份实现，
// 迟早分叉，而分叉的那天前端会拦住后端本来接受的名字，或者放过后端拒绝的名字。
// 这里只负责把用户敲的东西原样送到，让 400 带着后端自己的措辞回到用户眼前。
